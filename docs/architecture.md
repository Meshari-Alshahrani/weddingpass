...
| **`/api/admin`** | API Handler | مسار إدارة الفعالية واعتماد وحذف الصور والتهاني (جلسة مشرف + مطابقة eventId إلزامية) |
| **`/api/admin/auth`** | API Handler | إصدار/إنهاء جلسة الإدارة الموقعة HMAC (ADMIN_PIN مستقل + كوكي `__Host-admin_session`) |
| **`/api/calendar`** | API Handler | مولد ملفات التقويم RFC 5545 لأجهزة Apple و Google (تهريب نصوص وطي 75-octet) |
...
4. **نظام الترحيل وقاعدة البيانات المعتمد:**
   - ملفات `supabase/migrations/` من `001` إلى `007` هي الدليل التشغيلي الوحيد لنشر وتحديث قاعدة البيانات.

5. **طبقات الحماية الإدارية (v6.0.0):**
   - `proxy.ts` فحص تفاؤلي مبكر (تحويل سريع) + Hardened CSP بنون عبر `CSP_MODE`.
   - `requireAdminSession()` في الـDAL قبل أي استعلام حساس — الحماية الفعلية.
   - كل handler إداري يعيد التحقق ويطابق `eventId` المستخرج من الجلسة الموقعة.
